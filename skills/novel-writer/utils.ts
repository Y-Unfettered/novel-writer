...
function cleanCreatureCandidate(candidate: string): string {
...
function isLikelyCreatureCandidate(candidate: string, knownNames: Set<string>): boolean {
...
export function extractLikelyCreatureNames(text: string, existingNames: string[] = []): string[] {
  if (!text.trim()) {
    return [];
  }
