...
class SummaryUpdater {
     ...
     */
    buildCreatureCardPrompt(creatureName, context) {
        return `## 生物卡片生成任务
...
4. 典籍记载需符合《山海经》的语言风格`;
     ...
     */
    buildCreatureSummaryPrompt(creatureName, creatureCard) {
        return `## 生物简述生成任务
...
"此兽名曰XXX，形体如XX，其色XX。见之则避，莫敢前也。其性XX，好食XX。有XX之能，若遇之，可XX以对。"`;
     ...
     */
    buildAutoCreatureCardPrompt(currentCreatureCards, chapterContent, newCreatureName) {
        return `## 自动生成新生物卡片任务
...
- 该生物的危险等级是否值得记录`;
     ...
     */
    buildDangerLevelUpdatePrompt(creatureName, currentCreatureCard, newChapterSummary) {
        return `## 危险等级评估更新任务
